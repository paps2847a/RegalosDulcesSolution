...
namespace ClienteRegalosDulces.Controllers
{
    ...
    public class InventarioController : Controller
    {
        ...
        private readonly ICategoriaService _svcCat;

        public InventarioController(IInventarioService InventarioService, ICategoriaService svcCat)
        {
            _svc = InventarioService ?? throw new ArgumentNullException(nameof(InventarioService));
            _svcCat = svcCat;
        }
        ...
        {
            ViewBag.Categories = await _svcCat.GetAllAsync(new Filter());
            return View(id > 0 ? await _svc.GetItem(id) : new Inventario());
        }
        ...
        public async Task<IActionResult> Save([FromForm] Inventario model)
        {
            try
            {
                var r = new Inventario();
                if (model.IdInv == 0)
                {
