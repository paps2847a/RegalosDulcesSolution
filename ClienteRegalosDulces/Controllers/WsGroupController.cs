...
namespace ClienteRegalosDulces.Controllers
{
    ...
    public class WsGroupController : Controller
    {
        private readonly IWsGroupService _svc;

        public WsGroupController(IWsGroupService WsGroupService)
        {
            _svc = WsGroupService ?? throw new ArgumentNullException(nameof(WsGroupService));
        }
        ...
        {
            return View(id > 0 ? await _svc.GetItem(id) : new WsGroup());
        }
        ...
        public async Task<IActionResult> Save([FromForm] WsGroup model)
        {
            try
            {
                var r = new WsGroup();
